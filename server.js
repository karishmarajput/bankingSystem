...
app.get('/transfer',(req,res)=>{
    ...
    customer.find({ accountNo: { $ne: acc } },function (err, users) {
        if (err) {
            ...
            res.render("transfer", {accountNo: `${acc}`,name: `${name}`,email: `${email}`, users: users })
        }
    })
})
    ...
    customer.findOne({ accountNo: obj.customer1 },function (err, user) {
        if (err) {
            ...
            if(parseInt(user.balance) < parseInt(obj.amount)){
                ...
                customer.findOneAndUpdate({accountNo:obj.customer2}, {$inc: { "balance" : parseInt(obj.amount) }},function(err,res){
                    if (err) throw err;
                    console.log("customer1 updated");
                })
                customer.updateOne({accountNo:obj.customer1},{balance: parseInt(user.balance)-parseInt(obj.amount)},function(err,res){
                    if (err) throw err;
